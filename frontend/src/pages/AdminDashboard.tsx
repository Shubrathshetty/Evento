...
const AdminDashboard = () => {
  ...
  const handleApproveRegistration = async (registrationId: string) => {
    ...
    try {
      await registrationsService.updateRegistrationStatus(
        registrationId,
        'approved',
        user.id
      );
      ...
      setRegistrations(prev =>
        prev.map(reg =>
          reg.id === registrationId
            ? { ...reg, status: 'approved', approved_at: new Date().toISOString(), approved_by: user.id }
            : reg
        )
      );
      ...
      toast({
        ...
        description: "The registration has been approved and a ticket has been issued.",
      });
    } catch (error: any) {
      toast({
  ...
  const handleRejectRegistration = async (registrationId: string) => {
    ...
    try {
      ...
      setRegistrations(prev =>
        prev.map(reg =>
          reg.id === registrationId
            ? { ...reg, status: 'rejected', approved_by: user.id }
            : reg
        )
      );
